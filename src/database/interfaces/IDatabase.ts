...
export interface IDatabase {
  ...
  updateScoreEvent(delegatee: string, blockNumber: number, update: Partial<ScoreEvent>): Promise<void>;
  deleteScoreEvent(delegatee: string, blockNumber: number): Promise<void>;
  deleteScoreEventsByBlockRange(fromBlock: number, toBlock: number): Promise<void>;
  getScoreEvent(delegatee: string, blockNumber: number): Promise<ScoreEvent | null>;
  getLatestScoreEvent(delegatee: string): Promise<ScoreEvent | null>;
  getScoreEventsByBlockRange(fromBlock: number, toBlock: number): Promise<ScoreEvent[]>;
}
